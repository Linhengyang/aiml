...
cdef extern from "mt_pair_count_merge.h":
    ...
    struct u16token_pair_counts_ptrs:
     ...
     # 声明 C++ 中的 c_tls_count_u16pair_batch 函数 nogil
    ...
    # 声明 C++ 中的 c_merge_u16pair_batch 函数 nogil
...
cpdef count_u16pair_batch(
    ...
    cdef cnp.ndarray[cnp.uint16_t, ndim=1, mode="c"] tokens_flat = tokens_offsets[0]
    ...
    cdef cnp.ndarray[cnp.uint16_t, ndim=1, mode="c"] L_tokens = tokens_flat[mask] # 可以为空
    
    # 去掉所有 chunk 开头的 token, 就是所有 R_tokens
    mask_cp[chunk_starts_] = False
    cdef cnp.ndarray[cnp.uint16_t, ndim=1, mode="c"] R_tokens = tokens_flat[mask_cp] # 可以为空
    ...
    cdef u16token_pair_counts_ptrs result
    cdef size_t size

    cdef uint16_t *raw_L_ptr, *raw_R_ptr
    ...
    # ----- NOGIL 区域: 核心计算 -----
    with nogil:
        # 调用 c_tls_count_u16pair_batch, 对 u16-token batch data 计算 pair-counts
        result = c_tls_count_u16pair_batch(
            L_tokens_ptr,
            R_tokens_ptr,
            len
        )
...
cpdef merge_u16pair_batch(
    ...
    if num_chunks <= 0:
        return (np.array([], dtype=np.uint16), np.array([0], dtype=np.int64))
    ...
    cdef merged_u16token_offset_ptrs result
    ...
    cdef uint16_t* raw_tokens_ptr
    cdef uintptr_t tokens_addr

    # ----- NOGIL 区域: 核心计算 -----
    with nogil:
        result = c_tls_merge_u16pair_batch(
            tokens_flat_ptr,
            offsets_ptr,
            num_chunks,
            pair_L,
