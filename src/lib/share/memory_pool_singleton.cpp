...
// 定义静态成员变量
// 用智能指针来管理单例
std::unique_ptr<threadsafe_singleton_mempool, threadsafe_singleton_mempool::Deleter> threadsafe_singleton_mempool::_instance = nullptr;
std::mutex threadsafe_singleton_mempool::_mtx;




// 构造函数. 放到 private 里导致它不对外使用.
threadsafe_singleton_mempool::threadsafe_singleton_mempool(size_t block_size, size_t alignment):
...
threadsafe_singleton_mempool::~threadsafe_singleton_mempool() {
    // 析构时调用不加锁的 release_no_lock 方法. 因为析构已经被私有, 不用担心析构被公开调用; 而释放内存池的公共接口 destroy 会带锁
    // 所以为了避免死锁, 这里无需上锁
    release_no_lock();
}
...
void threadsafe_singleton_mempool::release_no_lock() {
    // 不加锁的 release 内存池, 给加锁的release等外部函数使用 --> 必须在外部加锁使用
...
// 动态成员方法, 提供内存池释放的接口给实例使用 --> instance.release()
void threadsafe_singleton_mempool::release() {

    // public方法 release 涉及到修改共享变量, 加锁只能由一个线程执行.
    std::lock_guard<std::mutex> lock(_mtx);

    threadsafe_singleton_mempool::release_no_lock();
...
// 申请分配 size 字节数量的内存.
void* threadsafe_singleton_mempool::allocate(size_t size) {
...
void threadsafe_singleton_mempool::dealloc_large(void* ptr) {
    // 单独释放大内存, 加互斥锁
    std::lock_guard<std::mutex> lock(_mtx);
...
void threadsafe_singleton_mempool::reset() {
...
// 下一步紧接着就是 reset
void threadsafe_singleton_mempool::shrink(size_t max_num) {
    std::lock_guard<std::mutex> lock(_mtx);
...
std::unique_ptr<singleton_mempool, singleton_mempool::Deleter> singleton_mempool::_instance = nullptr;


singleton_mempool::singleton_mempool(size_t block_size, size_t alignment):
    _block_size(block_size),
    _blocks(std::vector<block*>()),
    _large_allocs(std::vector<void*>()),
    _alignment(alignment)
...
void singleton_mempool::release() {
...
void* singleton_mempool::allocate(size_t size) {
...
void singleton_mempool::dealloc_large(void* ptr) {
...
void singleton_mempool::shrink(size_t max_num) {
