...
public:
    ...
    // 万能引用(模板+&&引用+完美转发): 本质是按值传参的极致优化版(形参被直接优化掉了, 不生成对象, 只是引用), 在函数体内部以 std::forward<U>(形参) 的方式保持实参的属性，透传给底层. 实参既可以是左值，也可以是右值
    //   但是, 当实参是左值时, 按值传参用形参隔离了实参, 所以修改形参并不会改动实参; 而万能引用中, 形参并没有生成只是实参的引用, 所以是存在改变实参的风险的, 好在大部分使用形参的方法是用其触发const T&签名的方法, 以触发拷贝.
    //   万能引用函数内部, 形参的黄金用法是: 不修改, 不提前move, 只作只读操作(触发const T&参数的方法), 并只在最后一次通过 std::forward 转发(数据转移or拷贝)给底层.
    // 左值引用: 实参必须是左值, 一般用于需要修改实参的场景
    ...
    // 返回右值引用 / 返回指针: 都是基本不考虑的选项
    //   返回右值引用 只有在一些底层库还有丁点黑魔法用处
    //   返回指针 基本用于可空返回/多态/兼容C等小众地方
