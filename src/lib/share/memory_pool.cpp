...
// 构造函数
threadsafe_mempool::threadsafe_mempool(size_t block_size, size_t alignment):
...
// 析构函数
threadsafe_mempool::~threadsafe_mempool() {
    release();
}
...
// 申请分配 size 字节数量的内存.
void* threadsafe_mempool::allocate(size_t size) {
...
void threadsafe_mempool::dealloc_large(void* ptr) {
    // 单独释放大内存, 加互斥锁
    std::lock_guard<std::mutex> lock(_mtx);
...
// 下一步紧接着就是 reset
void threadsafe_mempool::shrink(size_t max_num) {
    std::lock_guard<std::mutex> lock(_mtx);
...
void threadsafe_mempool::release() {
    std::lock_guard<std::mutex> lock(_mtx);
