...
// 构造函数
mempool::mempool(size_t block_size, size_t alignment):
...
// 下一步紧接着就是 reset
void mempool::shrink(size_t max_num) {
    std::lock_guard<std::mutex> lock(_mtx);
...
// 构造函数
un_threadsafe_mempool::un_threadsafe_mempool(size_t block_size, size_t alignment):
    _block_size(block_size),
    _blocks(std::vector<block*>()),
    _large_allocs(std::vector<void*>()),
    _alignment(alignment)
...
// 析构函数
un_threadsafe_mempool::~un_threadsafe_mempool() {
    release();
}
...
// 申请分配 size 字节数量的内存.
void* un_threadsafe_mempool::allocate(size_t size) {
...
//如果 ptr是大对象(大于_block_size)的，会被释放;否则不会被释放
//如果ptr在 _large_allocs 里，那么释放它，然后从 _large_allocs 中删除它。否则不作任何操作
void un_threadsafe_mempool::dealloc_large(void* ptr) {
    // 在 _large_allocs 中寻找 ptr
    auto it = std::find(_large_allocs.begin(), _large_allocs.end(), ptr);
    if(it != _large_allocs.end()) {
        std::free(ptr); // 找到了 ptr. 大对象单独释放
...
// 必须要在 reset之前用. 因为reset会重置所有block._offset=0
// 下一步紧接着就是 reset
void un_threadsafe_mempool::shrink(size_t max_num) {
    // 如果当前block指向空, 说明还没有创建任何一个block, 那么无可shrink，直接退出
    if(!_current_block) {
        return;
    }
...
// reset 内存池: reset 所有内存 block, release 所有 large_allocation
void un_threadsafe_mempool::reset() {
    for(auto block: _blocks) {
        block->reset();
    }
...
// 公共方法：释放内存池. 带锁以保证线程安全
void un_threadsafe_mempool::release() {
    for(auto block: _blocks) {
        // 释放 _blocks vector 中每一个内存block. 调用 block 的析构函数
        delete block;
    }
