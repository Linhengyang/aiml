...
class gpt2Tokenizer(boostBBPETokenizer):
    ...
    def bytes_to_unicode() -> dict[bytes, str]:
        ...
        控制字符和单空格  (共33个)   0   -   32  -->  unicode char 256-288 (+256映射)
        ASCII可打印字符            33   -   126 -->  unicode char 33 -126 (恒等映射)
        DEL和其他控制字符 (共34个)  127  -   160 -->  unicode char 383-416 (+256映射)
        Latin1可打印字符           161  -   172 -->  unicode char 161-172 (恒等映射)
        soft-hyphen字符 (共1个)        173      -->  unicode char 429     (+256映射)
        Latin1可打印字符            174 -   255 -->  unicode char 174-255 (恒等映射)
        
        即 68 个控制字符 + 空格, 映射成 256 + byte值 对应的 unicode 字符, 188 个可打印字符, 映射成 byte 值 对应的 unicode 字符(等价于原byte字符)
