...
def split_textfile(input_file_path, ratios:t.List[float], output_dir, shuffle:bool=True):
    ...
    for i in range(1, len(split_ends)):
        ...
        # 当前文件名
        cur_part_path = os.path.join(output_dir, str(r).rstrip('0').rstrip('.')+file_ext) 

        with open(cur_part_path, 'w') as f:
            # 把 split_ends[i-1]:split_ends[i] 这些行写入 当前文件
            f.writelines( lines[split_ends[i-1]:split_ends[i]] )
